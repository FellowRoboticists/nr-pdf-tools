module.exports = (function() {
  ...
  var pdfSeparateTemplate = compile("pdfseparate {0} {1}");
  ...
  var createWatermark = compile("convert -size {0}x{1} xc:transparent -font Helvetica-bold -pointsize 25 -fill \"graya(65%,0.5)\" -draw \"translate {2},{3} rotate -45 text 0,0 '{9}'\" -draw \"translate {4},{5} rotate -45 text 0,0 '{9}'\" -draw \"translate {6},{7} rotate -45 text 0,0 '{9}'\" {8}");
   ...
   * pageDir - full path to the directory in which to place the page PDFs.
   */
  var separateDocument = function(docToSplit, pageDir) {
    var cmd = pdfSeparateTemplate(quoteString(docToSplit), 
                                 path.join(pageDir, "page_%04d.pdf"));
    return pexec.pexec(cmd).
      then(function(res) { return pageDir; });
  };
  ...
  var createAWatermark = function(dimensions, watermarkFile, watermarkText) {
  ...
  var watermarkPdf = function(pdfToWatermark, watermarkText) {
