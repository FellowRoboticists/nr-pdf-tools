module.exports = (function() {
  ...
  var createWatermark = compile("convert -size {0},{1} xc:transparent -font Helvetica-bold -pointsize 25 -fill graya(85%,0.5) -draw \"translate {2},{3} rotate -45 text 0,0 'BIG HORN IMAGING'\" -draw \"translate {4},{5} rotate -45 text 0,0 'BIG HORN IMAGING'\" -draw \"translate {6},{7} rotate -45 text 0,0 'BIG HORN IMAGING'\" {8}");
  var watermarkToPdf = compile("convert {0} {1}");
  var stampPdf = compile("pdftk {0} stamp {1} output {2}");

  var textWidth = 250;
  var textAngleWidth = 177; // sqrt(textWidth^2 / 2)
  ...
  var createAWatermark = function(dimensions, watermarkFile) {
    console.log("Dimensions: %j", dimensions);
    var cmd = createWatermark(dimensions.imageSize.width,
  ...
  var convertWatermarkToPdf = function(watermarkPath, watermarkPdfPath) {
    var cmd = watermarkToPdf(watermarkPath, watermarkPdfPath);
    return pexec.pexec(cmd).
      then(function() {
        return watermarkPdfPath;
      });
  };

  var stampPdfWithWatermark = function(pdfToWatermark, watermarkPdf, resultingPdf) {
    var cmd = stampPdf(pdfToWatermark, watermarkPdf, resultingPdf);
    return pexec.pexec(cmd).
      then(function() {
        return resultingPdf;
      });
  ...
  var watermarkPdf = function(pdfToWatermark) {
    var watermarkFile = path.join(path.dirname(pdfToWatermark), "watermark.png");
    var watermarkPdfPath = path.join(path.dirname(pdfToWatermark), "watermark.pdf");
    var resultingPdf = path.join(path.dirname(pdfToWatermark), "watermarked.pdf");
    return identifyImageSize(pdfToWatermark).
      then(function(imageSize) {
        return {
          ...
          center: {
            x: (imageSize.width - textAngleWidth) / 2,
            y: imageSize.height / 2 + textAngleWidth
          },
          lowerRight: {
      ...
      then(function(dimensions) {
        return createWatermark(dimensions, watermarkFile);
      }).
      then(function(watermarkPath) {
        return convertWatermarkToPdf(watermarkPath, watermarkPdfPath);
      }).
      then(function(watermarkPdf) {
