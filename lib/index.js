...
module.exports = (function () {
   ...
   * pageDir - full path to the directory in which to place the page PDFs.
   */
  var separateDocument = (docToSplit, pageDir) => {
    let cmd = pdfSeparateTemplate(quoteString(docToSplit),
                                 path.join(pageDir, 'page_%04d.pdf'))
    return pexec.pexec(cmd)
      .then((res) => pageDir)
  }
  ...
  const uniteDocument = (dirName, inFiles, outFile) => {
    let cmd = pdfUniteTemplate(dirName,
                               inFiles.map(function (f) { return quoteString(f) }).join(' '),
                               quoteString(outFile))
  ...
  const pdfrwRotateDocument = (dirName, inFile, outFile, direction) => {
    ...
    let cmd = pdfRotateTemplate(dirName, inFile, rotate, outFile)
